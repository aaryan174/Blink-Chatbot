...
const SideBar = () => {
  ...
  const getAllThreads = async () => {
    try {
      ...
      //  stores threadId, title, messages
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const deleteThread = async(threadId) => {
    ...
    const res = await response.json();
    console.log(res);
    // adding funtionality that when we delete the threads there will no need to refresh the scren

    setAllThreads( prev => prev.filter(thread => thread.threadId !== threadId)); 

// check not to delete current thread 
  ...
  return (
    <>
      {sidebarOpen && <div className={`mobile-overlay ${!sidebarOpen ? 'hidden' : ''}`} onClick={() => setSidebarOpen(false)}></div>}
      ...
      <ul className="chathistory ">
        {allThreads?.map((thread, idx) => (
          <li
            className={thread.threadId === currThreadId ? "highlighted": "chat-item"}
